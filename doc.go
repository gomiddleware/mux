// Package mux is a simple(ish) mux, however, it provides a few nice features such as easy use of middleware and a
// router which doesn't automatically look at only prefixes (like the Go built-in mux). Everything is explicit and
// nothing is done for you. ie. we don't automatically redirec to a slash or non-slash version of each path, you need
// to do that yourself (or use github.com/gomiddleware/slash to help).
//
...
//
// An example of middleware calling next might be adding a requestId in to each request's Context. An
//
// An example of middleware dealing with the request might be to check that a user is logged in, and if not, sending a
// redirect to "/", or sending back a 403.
//
